...
builder.Services.AddAuthentication(JwtBearerDefaults.AuthenticationScheme).AddJwtBearer(o=>{
 ...
 o.Events=new(){OnMessageReceived=c=>{if(c.HttpContext.Request.Path.StartsWithSegments("/hubs/orders"))c.Token=c.Request.Query["access_token"];return Task.CompletedTask;},OnTokenValidated=async c=>{var db=c.HttpContext.RequestServices.GetRequiredService<RestaurantDb>();var id=c.Principal?.FindFirstValue(ClaimTypes.NameIdentifier);var role=c.Principal?.FindFirstValue(ClaimTypes.Role);if(!Guid.TryParse(id,out var uid)||!await db.Users.AnyAsync(u=>u.Id==uid&&u.Active&&u.Role==role))c.Fail("Account is inactive or role changed.");}};
});
...
app.MapPost("/api/auth/login",async(LoginRequest r,RestaurantDb db,IPasswordHasher<User> hasher)=>{
 var user=await db.Users.SingleOrDefaultAsync(x=>x.Email==r.Email.Trim().ToLower()&&x.Active);
 if(user==null||hasher.VerifyHashedPassword(user,user.PasswordHash,r.Password)==PasswordVerificationResult.Failed)return Results.Unauthorized();
 var expires=DateTime.UtcNow.AddHours(8);var claims=new[]{new Claim(ClaimTypes.NameIdentifier,user.Id.ToString()),new Claim(ClaimTypes.Name,user.Name),new Claim(ClaimTypes.Role,user.Role)};
 var token=new JwtSecurityToken(builder.Configuration["Jwt:Issuer"],builder.Configuration["Jwt:Audience"],claims,expires:expires,signingCredentials:new(new SymmetricSecurityKey(Encoding.UTF8.GetBytes(key)),SecurityAlgorithms.HmacSha256));
 return Results.Ok(new{token=new JwtSecurityTokenHandler().WriteToken(token),expires,user=new{user.Id,user.Name,user.Email,user.Role}});
...
api.MapPost("/sessions/{id:guid}/pay",async(Guid id,PayRequest r,RestaurantDb db,ClaimsPrincipal user,IHubContext<OrderHub> hub)=>{
 var tableId=await db.Sessions.Where(x=>x.Id==id).Select(x=>(int?)x.TableId).SingleOrDefaultAsync();if(tableId==null)return Results.NotFound();
 await using var tx=await db.Database.BeginTransactionAsync();await db.Tables.FromSqlInterpolated($"SELECT * FROM \"Tables\" WHERE \"Id\" = {tableId.Value} FOR UPDATE").SingleAsync();
 var existing=await db.Bills.SingleOrDefaultAsync(x=>x.SessionId==id);if(existing!=null)return Results.Ok(existing);
 var session=await db.Sessions.SingleAsync(x=>x.Id==id);var orders=await db.Orders.Include(x=>x.Items).Where(x=>x.SessionId==id).ToListAsync();if(session.ClosedAt!=null||orders.Count==0||orders.Any(x=>x.Status!="Served"))return Results.Conflict(new{message="All orders for this table must be served before payment."});
 var method=await db.PaymentMethods.SingleOrDefaultAsync(x=>x.Id==r.PaymentMethodId&&x.Active);if(method==null)return Bad("Choose an active payment method.");if((r.Reference?.Length??0)>120)return Bad("Payment reference must be 120 characters or fewer.");
 var total=orders.Sum(x=>x.Items.Sum(i=>i.Quantity*i.UnitPrice));if(total!=r.ExpectedTotal)return Results.Conflict(new{message="The bill changed. Review the latest total before collecting payment."});
 var bill=new Bill{SessionId=id,TableId=session.TableId,CashierId=UserId(user),Total=total,PaymentMethodId=method.Id,Reference=r.Reference?.Trim()??""};db.Bills.Add(bill);session.ClosedAt=DateTime.UtcNow;
 foreach(var order in orders){order.Status="Paid";order.UpdatedAt=DateTime.UtcNow;db.OrderEvents.Add(new(){OrderId=order.Id,UserId=UserId(user),Status="Paid"});}await db.SaveChangesAsync();await tx.CommitAsync();await Changed(hub);return Results.Ok(bill);
...
static async Task Changed(IHubContext<OrderHub> hub){try{await hub.Clients.All.SendAsync("StateChanged");}catch{/* Persisted state remains authoritative; clients resync on reconnect and periodically. */}}
