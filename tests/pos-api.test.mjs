...
test('POS migration, customization, ledger, audit, guests, and branch isolation',async()=>{
 ...
 assert.ok(bill.bill?.invoiceNumber,'Invoice number should be generated upon payment');
 assert.match(bill.bill.invoiceNumber,/^INV-\d{2}-\d{4}-\d{5}$/);
 await call('/pos/sessions/'+order.sessionId+'/refund','POST',{amount:300,reason:'Too much'},token,400);
 await call('/pos/sessions/'+order.sessionId+'/refund','POST',{amount:50,reason:'Guest compensation'},token);
 bill=await call('/pos/sessions/'+order.sessionId,'GET',undefined,token);
 assert.equal(bill.refunds.length,1);
 assert.equal(bill.refunds[0].amount,50);
 assert.equal(bill.quote.paid,157.9);
 assert.equal(bill.quote.outstanding,50);
 const healthRes=await fetch(base+'/health');
 assert.equal(healthRes.status,200);
 const healthJson=await healthRes.json();
 assert.equal(healthJson.database,'Healthy');
